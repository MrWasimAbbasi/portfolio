    ...
    <div id="filter-menu" class="w-full flex-col border-right font-fira_regular text-menu-text hidden lg:flex">
      ...
      <nav id="filters" class="w-full flex-col">
        ...
        <div v-for="tech in techs" :key="tech" class="flex items-center py-2">
          ...
          <img :id="'icon-tech-' + tech"
               :src="'/icons/techs/filled/' + tech.replace(/\s+/g, '-').toLowerCase() + '.svg'" alt=""
          ...
          <div id="project-card" class="flex flex-col">
            <div id="window">
              <div class="absolute flex right-3 top-3">
                <img :title="tech.title" v-for="tech in project.tech" :key="tech.icon"
                     :src="'/icons/techs/filled/' + tech.icon + '.svg'" alt="" class="w-6 h-6 mx-1 hover:opacity-75">
              </div>
            ...
            <div class="pb-8 pt-6 px-6 border-top">
              ...
              <a id="view-button" :href="project.url" target="_blank"
                ...
                {{ project.type === "Private" ? "Private & Confidential" : "view-project" }}
              </a>
            </div>
          </div>
        </div>
...
export default {
  ...
  data() {
    return {
      techs: ['Laravel', 'Vue', 'Yii', 'React', 'GraphQL', 'REST API'],
      filters: ['all'],
      projects: '',
      loading: true,
    };
  ...
  methods: {
    ...
    filterProjectsBy(filters) {
      ...
      return projectArray.filter(project => {
        let projectTechArray = project.tech
        return filters.some((filter) => {
          return projectTechArray.some((projectTech) => {
            return projectTech.title.includes(filter)
          })
        })
      });
    },
